            ...
            <div class="answer-section">
                ...
                @if(count($answers) !=0 )
                    ...
                    <div class="answer">
                        ...
                        ?>
                                    <a href="{{route('common.index', ['step'=>'first-step', 'type'=>$query->type, 'query_id'=>$sub_query->id, 'answer_id'=>$answer->id, 'category'=>$query->category])}}">
                                        <span class="num">{{$i}}</span>
                                        <span>{{$answer->answer_content}}</span>
                                        <img class="arrow" src={{asset('public/assets/images/arrow.png')}}>
                                    </a>    
                        ...
                        ?>
                                    <a href="{{route('common.index', ['step'=>'choose', 'type'=>$query->type, 'query_id'=>$sub_query->id, 'query_content'=>$sub_query->query_content, 'index'=>0])}}">
                                        <span class="num">{{$i}}</span>
                                        <span>{{$answer->answer_content}}</span>
                                        <img class="arrow" src={{asset('public/assets/images/arrow.png')}}>
                                    </a>    
            ...
            success: function (data) {                       
                if(data.success){
                    ...
                    if(data.final_template){  
                        url = '{{route("common.index", ["step"=>"last-step", "template_id"=>":template_id", "template_query"=>":template_query" ])}}';
                        ...
                        url = '{{route("common.index", ["step"=>"choose", "index"=>":index", "type"=>":type", "query_id"=>":query_id", "query_content"=>":query_content" ])}}';
                        url = decodeURIComponent(url);
                ...
                if(data.success){
                    ...
                    if(last_file != null){
                        if(last_file.indexOf('.jpg') > 0 || last_file.indexOf('.png') > 0 || last_file.indexOf('.jpeg') > 0 || last_file.indexOf('.JPG') > 0 || last_file.indexOf('.PNG') > 0 || last_file.indexOf('.JPEG') > 0){
                            ...
                            setTimeout("window.location.href = '{{route('home')}}'",parseInt(redirectPauseSeconds*1000));
                        }
                    }
                    if(last_url != null){
